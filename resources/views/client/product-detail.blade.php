...
@section('content')
    ...
    <main class="max-w-[1440px] mx-auto px-4 md:px-10 lg:px-20 py-8 lg:py-12 relative min-h-screen">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('client.products.index') }}"
                            class="hover:text-primary transition-colors">{{ $product->categories->first()?->name ?? 'Sản phẩm' }}</a>
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        <span class="material-symbols-outlined text-[16px] mx-1">chevron_right</span>
                        <span
                            class="text-[#181611] dark:text-white font-bold truncate w-48 sm:w-auto">{{ $product->name }}</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="mb-4">
            <h1 class="text-2xl lg:text-3xl font-bold text-[#181611] dark:text-white">
                {{ $product->name }}
            </h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <section class="lg:col-span-8" data-purpose="product-gallery">
                <div
                    ...
                    @php
                        ...
                        if ($mainImg) {
                            $mainUrl = Str::startsWith($mainImg, ['http://', 'https://'])
                                ? $mainImg
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="flex flex-col gap-3 max-h-[420px] overflow-y-auto">
                            ...
                            <button class="thumb-btn w-16 h-16 border-2 border-primary rounded-lg p-1">
                                <img src="{{ $mainUrl }}" class="w-full h-full object-contain" />
                            </button>
                            ...
                            @foreach ($product->images as $gallery)
                                @php
                                    ...
                                    $galUrl = $galImg
                                        ? (Str::startsWith($galImg, ['http://', 'https://'])
                                            ? $galImg
                                            : asset('storage/' . $galImg))
                                        : 'https://placehold.co/200x200';
                                @endphp

                                <button class="thumb-btn w-16 h-16 border rounded-lg p-1 hover:border-primary">
                                    <img src="{{ $galUrl }}" class="w-full h-full object-contain" />
                                </button>
                            @endforeach
                        </div>
                        ...
                        <div class="flex-1">
                            <div
                                class="aspect-square max-h-[420px] flex items-center justify-center rounded-xl bg-gray-50 p-2 border">
                                <img id="main-product-image" src="{{ $mainUrl }}" class="max-h-full object-contain" />
                            </div>
                        </div>
            ...
            <section class="lg:col-span-4" data-purpose="product-info-actions">
                <form action="#" method="POST" id="add-to-cart-form" class="flex flex-col gap-4">
                    ...
                    <div
                        class="bg-white dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/10 custom-shadow space-y-6">
                        @php
                            $groupedAttributes = [];
                            $variantsJS = [];
                            ...
                            if ($product->type == 'variable' && $activeVariants->isNotEmpty()) {
                                foreach ($activeVariants as $variant) {
                                    ...
                                    $variantsJS[] = [
                                        ...
                                        'specs' => $variant->specifications->mapWithKeys(
                                            fn($s) => [$s->spec_key => $s->spec_value],
                                        ),
                                    ];
                                }
                            }
                        ...
                        @if (!empty($groupedAttributes))
                            @foreach ($groupedAttributes as $attrName => $values)
                                <div class="attr-group" data-name="{{ $attrName }}">
                                    <p
                                        class="font-bold mb-3 text-[#181611] dark:text-white text-xs uppercase mb-2 tracking-wider">
                                        {{ $attrName }}:</p>
                                    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                                        @foreach ($values as $valId => $valName)
                                            <button type="button"
                                                class="attr-btn border border-gray-200 dark:border-white/10 bg-white dark:bg-black/20 rounded-lg py-1.5 px-2 text-xs font-medium hover:border-primary transition-all"
                                                data-id="{{ $valId }}">
                                                <div
                                                    class="check-icon absolute bottom-0 right-0 bg-[#f4c025] w-5 h-5 rounded-tl-xl flex items-center justify-center hidden">
                                                    <span
                                                        class="material-symbols-outlined text-[13px] fonte-bold text-[#181611] mr-[1px] mb-[1px]">check</span>
                                                </div>
                                                <span
                                                    class="block font-semibold text-sm text-gray-600 dark:text-gray-300 attr-text transition-colors relative z-10">{{ $valName }}</span>
                                            </button>
                                        @endforeach
                                    </div>
                                </div>
                    ...
                    <div
                        class="bg-white dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/10 custom-shadow">
                        <div class="flex items-center justify-between">
                            <p class="font-bold text-[#181611] dark:text-white">Số lượng:</p>
                            <div
                                class="flex items-center border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden focus-within:border-primary transition-colors bg-gray-50 dark:bg-black/20">
                                <button type="button" id="btn-minus"
                                    class="px-4 py-2 hover:bg-primary hover:text-black font-bold text-lg transition-colors">-</button>
                                <input type="number" id="input-qty" name="quantity" value="1" min="1"
                                    class="w-12 text-center border-0 focus:ring-0 p-2 font-bold bg-transparent text-[#181611] dark:text-white"
                                    readonly>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-12">
            <section class="lg:col-span-8 space-y-8">
                <div
                    ...
                    <h2
                        class="text-2xl font-bold mb-4 pb-2 border-b-2 border-primary inline-flex items-center gap-2 uppercase text-[#181611] dark:text-white">
                        <span class="material-symbols-outlined text-primary">article</span> Đánh giá chi tiết
                    </h2>
                    <div class="prose prose-slate dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 leading-relaxed overflow-hidden"
                        id="product-content" style="max-height: 800px;">
                        {!! $product->description !!}
                    </div>
                    <div class="mt-8 text-center relative">
                        <div class="absolute bottom-full left-0 w-full h-32 bg-gradient-to-t from-white dark:from-[#221e10] to-transparent"
                            id="content-gradient"></div>
                        <button id="read-more-btn"
                            class="text-[#181611] dark:text-white font-bold px-8 py-3 rounded-full border-2 border-gray-200 dark:border-white/20 hover:border-primary hover:bg-primary hover:text-black transition-all relative z-10 bg-white dark:bg-[#221e10]">Xem
                            thêm</button>
                    </div>
                </div>
            </section>
            ...
            <section class="lg:col-span-4">
                <div
                    ...
                    <h2
                        class="text-xl font-bold mb-4 pb-2 border-b-2 border-primary inline-flex items-center gap-2 uppercase text-[#181611] dark:text-white">
                        ...
                        @if ($displaySpecs->isNotEmpty())
                            <div class="w-full">
                                @foreach ($displaySpecs as $spec)
                                    <div
                                        class="spec-row p-3 flex justify-between border-b border-gray-100 dark:border-white/5 last:border-0">
                                        <span
                                            class="text-sm text-gray-500 dark:text-gray-400 w-1/3 font-medium">{{ $spec->spec_key }}:</span>
                                        <span
                                            class="text-sm font-bold text-[#181611] dark:text-white text-right w-2/3">{{ $spec->spec_value }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <div class="p-8 text-center flex flex-col items-center gap-2">
                                <span
                                    class="material-symbols-outlined text-4xl text-gray-300 dark:text-gray-600">inventory_2</span>
        ...
        @if ($relatedProducts->isNotEmpty() || $sameBrandProducts->isNotEmpty() || $fallbackProducts->isNotEmpty())
            <div class="mt-20" id="similar-products-section">
                ...
                <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
                    <div>
                        @if ($fallbackProducts->isNotEmpty())
                            ...
                            <h2 class="text-2xl lg:text-3xl fonte-bold text-[#181611] dark:text-white">Có thể bạn thích
                            </h2>
                        @else
                            <p class="text-xs font-bold text-primary uppercase tracking-widest mb-1">Gợi ý cho bạn</p>
                            <h2 class="text-2xl lg:text-3xl fonte-bold text-[#181611] dark:text-white">Sản phẩm tương tự
                            </h2>
                        @endif
                    </div>
                    <a href="{{ route('client.products.index') }}"
                ...
                @if ($relatedProducts->isNotEmpty() && $sameBrandProducts->isNotEmpty())
                    <div class="flex gap-2 mb-4" id="similar-tabs">
                        <button
                            class="similar-tab-btn active px-5 py-2 rounded-full text-sm font-bold border-2 border-primary bg-primary text-black transition-all"
                            data-tab="category">
                            <span class="material-symbols-outlined text-[15px] align-middle mr-1"
                                style="font-variation-settings:'FILL' 1">category</span>
                            Cùng danh mục
                        </button>
                        <button
                            class="similar-tab-btn px-5 py-2 rounded-full text-sm font-bold border-2 border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 hover:border-primary hover:text-primary transition-all"
                            data-tab="brand">
                            <span class="material-symbols-outlined text-[15px] align-middle mr-1"
                ...
                @if ($relatedProducts->isNotEmpty())
                    <div class="similar-tab-panel" id="panel-category">
                        <div class="relative">
                            {{-- Prev/Next buttons --}}
                            <button id="cat-prev"
                                class="absolute -left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-white dark:bg-[#2a2412] rounded-full shadow-lg border border-gray-100 dark:border-white/10 flex items-center justify-center hover:bg-primary hover:border-primary hover:text-black transition-all opacity-0 pointer-events-none"
                                aria-label="Trước">
                                <span class="material-symbols-outlined text-[20px]">chevron_left</span>
                            </button>
                            <button id="cat-next"
                                class="absolute -right-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-white dark:bg-[#2a2412] rounded-full shadow-lg border border-gray-100 dark:border-white/10 flex items-center justify-center hover:bg-primary hover:border-primary hover:text-black transition-all"
                                aria-label="Tiếp">
                            ...
                            <div class="overflow-hidden rounded-2xl" id="cat-slider-wrapper">
                                <div class="flex gap-4 transition-transform duration-500 ease-in-out"
                                    id="cat-slider-track" style="width: max-content;">
                                    @foreach ($relatedProducts as $relProduct)
                                        <div class="similar-card w-[220px] sm:w-[240px]">
                                            @include('client.home.partials.product-card', [
                                                'product' => $relProduct,
                                            ])
                                        </div>
                                    @endforeach
                ...
                @if ($fallbackProducts->isNotEmpty())
                    <div id="panel-fallback">
                        <div class="relative">
                            {{-- Prev/Next buttons --}}
                            <button id="fallback-prev"
                                class="absolute -left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-white dark:bg-[#2a2412] rounded-full shadow-lg border border-gray-100 dark:border-white/10 flex items-center justify-center hover:bg-primary hover:border-primary hover:text-black transition-all opacity-0 pointer-events-none"
                                aria-label="Trước">
                                <span class="material-symbols-outlined text-[20px]">chevron_left</span>
                            </button>
                            <button id="fallback-next"
                                class="absolute -right-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-white dark:bg-[#2a2412] rounded-full shadow-lg border border-gray-100 dark:border-white/10 flex items-center justify-center hover:bg-primary hover:border-primary hover:text-black transition-all"
                                aria-label="Tiếp">
                                <span class="material-symbols-outlined text-[20px]">chevron_right</span>
                            </button>
                            ...
                            <div class="overflow-hidden rounded-2xl" id="fallback-slider-wrapper">
                                <div class="flex gap-4 transition-transform duration-500 ease-in-out"
                                    id="fallback-slider-track" style="width: max-content;">
        ...
        <style>
            ...
            .similar-tab-btn:not(.active) {
                background: transparent;
            }

            #cat-slider-wrapper::-webkit-scrollbar {
                display: none;
            }
            ...
            (function() {
                ...
                (function initFallbackSlider() {
                    ...
                    function fGoTo(i) {
                        ...
                        fDots.querySelectorAll('.dot-btn').forEach((d, idx) => d.classList.toggle('active', idx ===
                            fCur));
                    }
                    ...
                    let fStartX = 0;
                    fWrapper.addEventListener('touchstart', e => {
        ...
        <div class="mt-16" id="reviews-section">
            ...
            <div
                class="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 p-4 mb-4 custom-shadow">
                    ...
                    <div class="flex flex-col items-center gap-3 min-w-[140px]">
                        <div class="text-center">
                            ...
                            <div class="flex justify-center gap-0.5 mt-2">
                                @for ($i = 1; $i <= 5; $i++)
                                    <span
                                        class="material-symbols-outlined text-[22px] {{ $i <= round($avgRating) ? 'text-primary' : 'text-gray-300 dark:text-gray-600' }}"
            ...
            {{-- ===== FILTER PILLS ===== --}}
            <div class="flex flex-wrap gap-2 mb-4" id="review-filters">
                <button
                    class="review-filter-btn active px-4 py-1.5 rounded-full border text-sm font-semibold
                ...
                <button
                    class="review-filter-btn px-4 py-1.5 rounded-full border text-sm font-semibold
                ...
                <div class="flex flex-col" id="review-list">
                    @foreach ($reviews as $review)
                        <div class="review-card py-6 border-b border-gray-100 dark:border-white/10 last:border-0"
                            id="review-{{ $review->id }}" data-rating="{{ $review->rating }}"
                                ...
                                @endif --}}
                                    ...
                                    <div class="flex items-center gap-2 mb-3">
                                        <span class="flex gap-0.5">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <span
                                                    class="material-symbols-outlined text-[15px] {{ $i <= $review->rating ? 'text-primary' : 'text-gray-200 dark:text-gray-600' }}"
                                                    style="font-variation-settings:'FILL' 1">star</span>
                                            @endfor
                                        </span>
                                    ...
                                    <div class="flex items-center gap-4 mt-1 mb-2">
                                        ...
                                        @endphp
                                        <button type="button"
                                            class="btn-helpful text-xs font-semibold {{ $isLiked ? 'text-primary' : 'text-gray-400' }} hover:text-primary transition-colors flex items-center gap-1"
                                            data-url="{{ route('reviews.helpful', $review->id) }}">
                                    ...
                                    @if ($review->hasReply())
                                        <div
                                            class="mt-3 pl-4 border-l-2 border-primary/50 bg-primary/5 dark:bg-primary/10 rounded-r-xl py-2.5 pr-3">
                                            <p class="text-xs font-bold text-primary flex items-center gap-1 mb-1">
                                                ...
                                                @if ($review->replied_at)
                                                    <span class="font-normal text-gray-400">•
                                                        {{ $review->replied_at->diffForHumans() }}</span>
                                                @endif
                                            </p>
                                            <p class="text-sm text-gray-700 dark:text-gray-300">
                                                {{ $review->reply_comment }}</p>
                                        </div>
                                    @endif
                                </div>
            ...
            @else
                <div
                    class="bg-white dark:bg-white/5 rounded-2xl p-12 border border-gray-100 dark:border-white/10 text-center custom-shadow">
                    <span class="material-symbols-outlined text-5xl text-gray-300 dark:text-gray-600">rate_review</span>
                    <p class="mt-3 text-gray-400 dark:text-gray-500">Chưa có đánh giá nào. Hãy là người đầu tiên đánh giá!
                    </p>
                </div>
            @endif
        </div>
        ...
        <div class="mt-16" id="qa-section">
            <div
                class="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 p-4 custom-shadow">
                    ...
                    <form action="{{ route('products.comments.store', $product->id) }}" method="POST" class="mb-8 block">
                        ...
                        <div class="flex flex-col gap-3">
                            <textarea name="content" rows="3" required
                                placeholder="Xin mời để lại câu hỏi, BeePhone sẽ phản hồi trong 1 giờ..."
                            ...
                            <div class="flex justify-end mt-1">
                                <button type="submit"
                                    class="px-6 py-2.5 bg-[#f4c025] text-[#181611] font-bold rounded-xl hover:brightness-105 transition-all text-sm shadow-[0_4px_14px_0_rgba(244,192,37,0.39)]">
                                    <span class="material-symbols-outlined text-[16px] inline-block align-middle mr-1"
                                        style="font-variation-settings: 'FILL' 1;">send</span> Gửi câu hỏi
                                </button>
                            </div>
                        </div>
                    </form>
                ...
                @guest
                    <div class="mb-8 block relative cursor-pointer" onclick="showLoginModal()">
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="flex justify-end mt-1">
                                <button type="button"
                                    class="px-6 py-2.5 bg-gray-300 dark:bg-gray-700 text-gray-500 font-bold rounded-xl pointer-events-none text-sm transition-all shadow-none">
                                    <span class="material-symbols-outlined text-[16px] inline-block align-middle mr-1"
                                        style="font-variation-settings: 'FILL' 1;">send</span> Gửi câu hỏi
                                </button>
                            </div>
                ...
                <div class="flex flex-col">
                    @forelse($comments as $comment)
                        <div class="py-6 border-b border-gray-100 dark:border-white/10 last:border-0 transition-all">
                            <div class="flex gap-4">
                                <div
                                    class="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold bg-[#181611] text-[#f4c025] shrink-0 shadow-inner">
                                    {{ mb_strtoupper(mb_substr($comment->user_id ? $comment->user->name ?? 'K' : ($comment->guest_name ?: 'K'), 0, 1)) }}
                                </div>
                                <div class="flex-1 min-w-0 text-sm">
                                    <div class="flex flex-wrap items-center gap-2 mb-1">
                                        <span
                                            class="font-bold text-[#181611] dark:text-white break-words">{{ $comment->user_id ? $comment->user->name ?? 'Khách' : ($comment->guest_name ?: 'Khách') }}</span>
                                        @if ($comment->user && $comment->user->role && $comment->user->role->name === 'admin')
                                            <span
                                                class="bg-[#f4c025] text-[#181611] text-[10px] font-bold px-1.5 py-0.5 rounded-sm uppercase tracking-wide">QTV</span>
                                        @endif
                                        <span class="text-xs text-gray-400 whitespace-nowrap">&bull;
                                            {{ $comment->created_at->diffForHumans() }}</span>
                                    </div>
                                    <p class="text-gray-700 dark:text-gray-300 leading-relaxed mb-2 break-words">
                                        {{ $comment->content }}</p>
                                    <div class="flex items-center gap-4 mt-2">
                                        <button type="button"
                                            class="text-xs text-gray-500 font-semibold hover:text-[#181611] dark:hover:text-[#f4c025] flex items-center gap-1 whitespace-nowrap transition-colors w-fit"
                                            onclick="@auth toggleReplyForm({{ $comment->id }}) @else showLoginModal() @endauth">
                                            <span class="material-symbols-outlined text-[14px]">reply</span> Phản hồi
                                        </button>
                                        @if (auth()->check() &&
                                                (auth()->id() === $comment->user_id || (auth()->user()->role && auth()->user()->role->name === 'admin')))
                                            <form action="{{ route('products.comments.destroy', $comment->id) }}"
                                                method="POST"
                                                onsubmit="return confirm('Bạn có chắc chắn muốn xóa không? Hành động này không thể hoàn tác.');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" title="Xóa bình luận"
                                                    class="text-xs text-red-400 font-semibold hover:text-red-600 flex items-center gap-1 whitespace-nowrap transition-colors w-fit">
                                    ...
                                    @auth
                                        ...
                                        <form action="{{ route('products.comments.store', $product->id) }}" method="POST"
                                            ...
                                            class="hidden mt-4 bg-[#f9f8f5] dark:bg-white/5 p-4 rounded-xl border border-[#e6e3db] dark:border-white/10 w-full">
                                            @csrf
                                            ...
                                            <div class="flex flex-col gap-3">
                                                ...
                                                <div class="flex justify-end gap-2 mt-1">
                                                    <button type="button"
                                                        class="px-3 py-1.5 text-xs text-gray-500 font-medium hover:bg-white dark:hover:bg-white/5 rounded-lg transition"
                                                        onclick="toggleReplyForm({{ $comment->id }})">Huỷ</button>
                                                    <button type="submit"
                                                        class="px-4 py-1.5 text-xs bg-[#f4c025] text-[#181611] font-semibold rounded-lg hover:brightness-105 transition shadow-sm">Gửi</button>
                                                </div>
                                            </div>
                                        </form>
                                    @endauth
                                    ...
                                    @if ($comment->replies->count() > 0)
                                        <div
                                            class="mt-4 flex flex-col gap-4 bg-[#f9f8f5] dark:bg-white/5 rounded-xl border border-[#e6e3db] dark:border-white/10 p-4">
                                            @foreach ($comment->replies as $reply)
                                                <div class="flex gap-4">
                                                    <div
                                                        class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 {{ $reply->user && $reply->user->role && $reply->user->role->name === 'admin' ? 'bg-[#181611] text-[#f4c025] shadow-inner' : 'bg-gray-400' }}">
                                                        {{ mb_strtoupper(mb_substr($reply->user_id ? $reply->user->name ?? 'K' : ($reply->guest_name ?: 'K'), 0, 1)) }}
                                                    </div>
                                                    <div class="flex-1 min-w-0 text-sm">
                                                        <div class="flex flex-wrap items-center gap-2 mb-1">
                                                            <span
                                                                class="font-bold text-[#181611] dark:text-white break-words">{{ $reply->user_id ? $reply->user->name ?? 'Khách' : ($reply->guest_name ?: 'Khách') }}</span>
                                                        ...
                                                        <div class="flex items-center gap-4 mt-1.5">
                                                            <button type="button"
                                                                class="text-xs text-gray-500 font-semibold hover:text-[#181611] dark:hover:text-[#f4c025] flex items-center gap-1 whitespace-nowrap transition-colors w-fit"
                                                                onclick="@auth focusReplyForm({{ $comment->id }}, '{{ $reply->user_id ? $reply->user->name ?? 'Khách' : ($reply->guest_name ?: 'Khách') }}') @else showLoginModal() @endauth">
                                                                <span
                                                                    class="material-symbols-outlined text-[14px]">reply</span>
                                                                Phản hồi
                                                            </button>
                                                            @if (auth()->check() &&
                                                                    (auth()->id() === $reply->user_id || (auth()->user()->role && auth()->user()->role->name === 'admin')))
                                                                <form
                                                                    action="{{ route('products.comments.destroy', $reply->id) }}"
                                                                    method="POST"
                                                                    onsubmit="return confirm('Bạn có chắc chắn muốn xóa phản hồi này không? Hành động này không thể hoàn tác.');">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit" title="Xóa phản hồi"
                                                                        class="text-xs text-red-400 font-semibold hover:text-red-600 flex items-center gap-1 whitespace-nowrap transition-colors w-fit">
                                                                        <span
                    ...
                    @empty
                        <div class="text-center py-6 text-gray-400">
                            <span
                                class="material-symbols-outlined text-4xl mb-2 text-gray-300 dark:text-gray-600">forum</span>
        ...
        <div id="login-modal-overlay"
            class="fixed inset-0 bg-black/60 z-[100] hidden flex items-center justify-center opacity-0 transition-opacity duration-300 backdrop-blur-sm">
            <div
                class="bg-white dark:bg-[#221e10] p-8 rounded-3xl shadow-2xl max-w-sm w-full mx-4 transform scale-95 transition-transform duration-300 relative border border-gray-100 dark:border-white/10">
                <button onclick="hideLoginModal()"
                    class="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition-colors bg-gray-50 dark:bg-white/5 rounded-full p-1.5 focus:outline-none">
                    <span class="material-symbols-outlined text-xl block">close</span>
                </button>
                <div class="text-center">
                    <div
                        class="w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-5 text-primary">
                        <span class="material-symbols-outlined text-4xl">lock</span>
                    </div>
                    <h3 class="text-2xl font-bold text-[#181611] dark:text-white mb-2">Yêu cầu đăng nhập</h3>
                    <p class="text-gray-500 dark:text-gray-400 text-sm mb-8 leading-relaxed">Hãy đăng nhập để sử dụng chức
                        năng này và tham gia thảo luận cùng cộng đồng Bee Phone.</p>
                    <div class="flex flex-col gap-3">
                        <a href="{{ route('login') }}"
                            class="w-full bg-[#f4c025] text-[#181611] font-bold py-3.5 rounded-xl hover:brightness-105 transition-all shadow-[0_4px_14px_0_rgba(244,192,37,0.39)] inline-block">Đăng
                            nhập ngay</a>
                        <button onclick="hideLoginModal()"
                            class="w-full bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-300 font-bold py-3.5 rounded-xl hover:bg-gray-200 dark:hover:bg-white/10 transition-colors">Để
                            sau</button>
                    </div>
                </div>
            </div>
    ...
    <script>
            ...
            thumbBtns.forEach(btn => {
                btn.addEventListener('click', function(e) {
                    ...
                    thumbBtns.forEach(b => {
                        ...
                        b.classList.add('border-gray-200', 'dark:border-white/10',
                            'border');
                    });
            ...
            function updateUI(price, salePrice, stock, image, variantId, sku, specs) {
                ...
                if (stock > 10) {
                    ...
                    stockStatusEl.className =
                        'text-xs text-green-600 bg-green-100 dark:bg-green-500/20 dark:text-green-400 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider';
                    stockTextEl.textContent = stock + ' máy';
                    stockTextEl.className = 'text-green-600 dark:text-green-400';
                    if (stockDotEl) {
                        stockDotEl.className = 'inline-block w-1.5 h-1.5 rounded-full bg-green-500';
                    }
                    ...
                    if (stockDotEl) {
                        stockDotEl.className = 'inline-block w-1.5 h-1.5 rounded-full bg-amber-500';
                    }
                    btnBuyNow.classList.remove('btn-disabled');
                    btnAddCart.classList.remove('btn-disabled');
                } else {
                    ...
                    stockStatusEl.className =
                        'text-xs text-red-600 bg-red-100 dark:bg-red-500/20 dark:text-red-400 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider';
                    stockTextEl.textContent = 'Hết hàng';
                    stockTextEl.className = 'text-red-600 dark:text-red-400';
                    if (stockDotEl) {
                        stockDotEl.className = 'inline-block w-1.5 h-1.5 rounded-full bg-red-500';
                    }
            ...
            if (productType === 'variable' && variantsList.length > 0) {
                ...
                function autoSelectMatching(clickedGroupName) {
                    ...
                    if (!currentMatch) {
                        ...
                        const newMatch = variantsList.find(v => v.stock > 0 && v.attributes.includes(
                                clickedValId)) ||
                ...
                function deselectButtonInGroup(group) {
                    group.querySelectorAll('.attr-btn').forEach(b => {
                        ...
                        b.classList.add('border-gray-100', 'dark:border-white/5', 'bg-white',
                            'dark:bg-black/20');
                        b.querySelector('.check-icon').classList.add('hidden');
                        b.querySelector('.attr-text').classList.remove('text-[#181611]',
                            'dark:text-[#f4c025]', 'fonte-bold');
                ...
                function selectButton(btn, group) {
                    ...
                    btn.querySelector('.attr-text').classList.remove('text-gray-600', 'dark:text-gray-300',
                        'font-semibold');
                    btn.querySelector('.attr-text').classList.add('text-[#181611]', 'dark:text-[#f4c025]',
                        'fonte-bold');
                }
                    ...
                    groups.forEach(group => {
                        ...
                        buttons.forEach(btn => {
                            ...
                            const isPossible = variantsList.some(variant => {
                                return testIds.every(attrId => variant.attributes.includes(
                                    attrId));
                            });
                            ...
                            if (isPossible) {
                                ...
                                if (!btn.querySelector('.strike-line')) {
                                    ...
                                    strike.className =
                                        'strike-line absolute inset-0 flex items-center justify-center pointer-events-none';
                                    strike.innerHTML =
                                        '<span style="display:block;width:80%;height:1.5px;background:currentColor;transform:rotate(-20deg);opacity:0.6;"></span>';
                ...
                function findMatchingVariant() {
                    ...
                    if (selectedIdsArr.length === 0) {
                        updateUI(basePrice, baseSalePrice, baseStock, null, '{{ $baseVariantId }}',
                            '{{ $product->sku }}', null);
                        return;
                    }
                    ...
                    let exactMatch = variantsList.find(variant => {
                        return selectedIdsArr.every(attrId => variant.attributes.includes(attrId)) &&
                            variant.attributes.length === selectedIdsArr
                            .length; // Số lượng thuộc tính phải bằng nhau
                    });
                    ...
                    if (selectedIdsArr.length < totalGroups) {
                        ...
                        stockStatusEl.className =
                            'text-xs text-amber-600 bg-amber-100 dark:bg-amber-500/20 dark:text-amber-400 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider';
                        ...
                        if (partialMatches.length > 0) {
                            let minPrice = Math.min(...partialMatches.map(m => m.sale_price > 0 && m.sale_price < m
                        ...
                        if (exactMatch || partialMatches.length > 0) {
                            ...
                            updateUI(matchedVariant.price, matchedVariant.sale_price, matchedVariant.stock,
                                matchedVariant.image, matchedVariant.id, matchedVariant.sku, matchedVariant
                                .specs);
                        } else {
                ...
                if (typeof updateUI === 'function') {
                    updateUI(basePrice, baseSalePrice, baseStock, null, '{{ $baseVariantId }}',
                        '{{ $firstVar->sku ?? $product->sku }}', @json($sSpecs));
                }
            }
        ...
        if (btnAddCart) btnAddCart.addEventListener('click', (e) => {
            e.preventDefault();
            handleAddToCart(false);
        });
        if (btnBuyNow) btnBuyNow.addEventListener('click', (e) => {
            e.preventDefault();
                ...
                filterBtns.forEach(function(btn) {
                    btn.addEventListener('click', function() {
                        ...
                        filterBtns.forEach(function(b) {
                            b.classList.remove('border-[#f4c025]', 'text-[#181611]',
                                'bg-[#f4c025]',
                                'shadow-[0_4px_10px_-2px_rgba(244,192,37,0.4)]', 'active');
                            b.classList.add('border-gray-200', 'text-gray-600', 'bg-white',
                                'dark:bg-white/5', 'dark:border-white/10',
                                'dark:text-gray-300');
                        });
                        this.classList.add('border-[#f4c025]', 'text-[#181611]', 'bg-[#f4c025]',
                            'shadow-[0_4px_10px_-2px_rgba(244,192,37,0.4)]', 'active');
                        this.classList.remove('border-gray-200', 'text-gray-600', 'bg-white',
                            'dark:bg-white/5', 'dark:border-white/10', 'dark:text-gray-300',
                            'hover:border-[#f4c025]', 'hover:text-[#f4c025]', 'hover:bg-[#f4c025]/5'
                        );
