...
<main class="flex-1 flex flex-col overflow-hidden px-8 bg-slate-50/50 dark:bg-slate-900/50">
    ...
    <div class="py-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Khởi tạo vai trò mới và định nghĩa các giới hạn quyền truy cập hệ thống</p>
        </div>
    ...
    <form action="{{ route('admin.role.store') }}" method="POST" class="space-y-6 max-w-7xl pb-10">
        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            @foreach ($modulePermissions as $permission)
                                <label class="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 cursor-pointer hover:border-primary/50 dark:hover:border-primary/50 transition-all">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="permission-checkbox w-4 h-4 rounded text-primary focus:ring-primary/30 border-slate-300 dark:border-slate-600 dark:bg-slate-900">
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
            <button type="reset" class="px-6 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                Làm lại
            </button>
