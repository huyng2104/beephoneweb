...
<div class="p-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Thêm Câu Hỏi Chatbot</h1>
        <p class="text-slate-600 dark:text-slate-400 text-sm mt-1">Tạo một câu hỏi và câu trả lời mới cho chatbot</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
        <form action="{{ route('admin.chatbot-faqs.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="question" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent @error('question') ring-2 ring-red-500 @enderror" placeholder="Nhập câu hỏi FAQ" value="{{ old('question') }}" required>
                @error('question')<span class="text-red-600 dark:text-red-400 text-sm mt-1 block">{{ $message }}</span>@enderror
                <small class="text-slate-500 dark:text-slate-400 text-xs mt-1.5 block">Ví dụ: "Bảo hành như thế nào?", "Giao hàng bao lâu?"</small>
            </div>
            ...
            <div>
                ...
                <textarea name="answer" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent @error('answer') ring-2 ring-red-500 @enderror" rows="8" placeholder="Nhập câu trả lời chi tiết cho câu hỏi" required>{{ old('answer') }}</textarea>
                @error('answer')<span class="text-red-600 dark:text-red-400 text-sm mt-1 block">{{ $message }}</span>@enderror
                <small class="text-slate-500 dark:text-slate-400 text-xs mt-1.5 block">Hãy viết câu trả lời rõ ràng, đầy đủ và dễ hiểu</small>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="priority" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="0" value="{{ old('priority', 0) }}">
                ...
                <div class="flex items-end">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" class="w-5 h-5 rounded border-slate-300 text-primary focus:ring-primary dark:border-slate-600 dark:bg-slate-700" checked value="1">
                        <span class="ml-3 text-sm font-medium text-slate-900 dark:text-white">Kích hoạt ngay</span>
                    </label>
                </div>
            </div>
