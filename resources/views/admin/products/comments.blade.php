                ...
                <div class="border-b border-slate-100 px-6 py-5 sm:px-8">
                    <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                        <div class="flex flex-1 items-start gap-5">
                            <div
                                class="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 sm:h-32 sm:w-32">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="mb-3 flex flex-wrap items-center gap-2">
                                    <span
                                        class="inline-flex items-center rounded-full bg-primary/15 px-3 py-1 text-xs font-bold uppercase tracking-wide text-slate-900">San
                                        pham</span>
                                    @if ($product->status === 'active')
                                        <span
                                            class="inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-800">Hien
                                            thi</span>
                                    @else
                                        <span
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <a href="{{ route('admin.products.index') }}"
                                class="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 shadow-sm transition hover:border-primary hover:text-primary">
                                <span class="material-symbols-outlined text-[18px]">arrow_back</span>
                                Quay lai danh sach san pham
                            </a>
                            <a href="#comments"
                                class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-slate-900 shadow-sm transition hover:brightness-105">
            ...
            <div id="comments" class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
                <div class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="comments-list px-6 py-2 sm:px-8">
                        ...
                        @empty
                            <div
                                class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-10 text-center text-sm font-medium text-slate-500">
                        ...
                        <form action="{{ route('products.comments.store', $product) }}" method="POST"
                            ...
                            @guest
                                <div>
                                    ...
                                    <input id="guest_name" type="text" name="guest_name" required
                                        class="comment-input" placeholder="Nhap ten hien thi">
                                </div>
                                <div>
                                    <label for="guest_email" class="comment-label">Email</label>
                                    <input id="guest_email" type="email" name="guest_email" class="comment-input"
                            ...
                            <div>
                                ...
                                <textarea id="content" name="content" rows="5" required class="comment-input comment-textarea"
                                    placeholder="Nhap noi dung comment cua ban..."></textarea>
                            </div>
                            <div>
