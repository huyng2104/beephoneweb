...
@section('content')
    <div class="w-full p-4 sm:p-8 space-y-6 font-display">
        ...
        <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
            <div class="flex flex-wrap justify-between items-end gap-4">
                <div class="flex flex-col gap-1">
                    ...
                    <p class="text-slate-500 text-sm">Quản lý kho hàng, cập nhật giá và thông tin các dòng điện thoại Bee
                        Phone của bạn.</p>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.products.trash') }}"
                        class="flex items-center justify-center rounded-lg h-11 px-4 bg-white border border-slate-200 text-slate-700 text-sm font-bold hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all shadow-sm gap-2">
                    ...
                    <a href="{{ route('admin.products.create') }}"
                        class="flex items-center justify-center rounded-lg h-11 px-5 bg-primary text-black text-sm font-bold hover:brightness-105 transition-all shadow-sm gap-2">
        ...
        @if(session('success'))
            <div
                class="p-4 bg-green-50 border border-green-200 text-green-700 rounded-xl text-sm font-bold flex items-center gap-2 shadow-sm">
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-900 leading-none">
                        {{ $products->where('status', 'active')->count() }}</p>
                </div>
            </div>
            <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-900 leading-none">
                        {{ $products->filter(fn($p) => $p->stock <= 5 && $p->stock > 0)->count() }}</p>
                </div>
            </div>
            <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-900 leading-none">{{ $products->where('stock', 0)->count() }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div class="relative w-80">
                    ...
                    <input
                        class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-slate-700 placeholder-slate-400 shadow-sm"
                        placeholder="Tìm kiếm theo tên hoặc mã SKU..." type="text" />
                </div>
                <div class="flex gap-3">
                    <button
                        class="px-4 py-2.5 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-2 text-slate-600 text-sm font-bold shadow-sm">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-600">
                    <thead
                        class="bg-slate-50 border-b border-slate-200 text-[11px] uppercase font-black text-slate-500 tracking-wider">
                        <tr>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors group">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="size-12 shrink-0 rounded-lg border border-slate-200 overflow-hidden bg-white flex items-center justify-center p-1 shadow-sm">
                                        ...
                                        <div class="flex flex-col">
                                            <span class="text-sm font-bold text-slate-900 line-clamp-1 mb-1"
                                                title="{{ $product->name }}">
                                                {{ $product->name }}
                                            </span>
                                            <div class="flex items-center gap-2">
                                                @if($product->type == 'variable')
                                                    <span
                                                        class="px-2 py-0.5 bg-blue-50 text-blue-600 border border-blue-100 rounded text-[10px] font-black uppercase tracking-wider">Biến
                                                        thể</span>
                                                @else
                                                    <span
                                                        class="px-2 py-0.5 bg-slate-100 text-slate-600 border border-slate-200 rounded text-[10px] font-black uppercase tracking-wider">Đơn
                                                        giản</span>
                                                @endif
                                                <span class="text-[11px] font-semibold text-slate-400">SKU:
                                                    {{ $product->sku ?? '---' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex flex-wrap gap-1.5">
                                        @forelse($product->categories as $category)
                                            <span
                                                class="text-[11px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded border border-slate-200">
                                                {{ $category->name }}
                                            </span>
                                        @empty
                                ...
                                <td class="py-4 px-6 whitespace-nowrap text-center">
                                    @if($product->status == 'active')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded text-[11px] font-black uppercase tracking-wider bg-green-100 text-green-700 border border-green-200">Hiển
                                            thị</span>
                                    @elseif($product->status == 'inactive')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded text-[11px] font-black uppercase tracking-wider bg-slate-100 text-slate-600 border border-slate-200">Ẩn</span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded text-[11px] font-black uppercase tracking-wider bg-red-100 text-red-700 border border-red-200">Lưu
                                            trữ</span>
                                    @endif
                                </td>
                                <td class="py-4 px-6 whitespace-nowrap text-right">
                                    <div
                                        ...
                                        <a href="{{ route('admin.products.show', $product->id) }}"
                                            class="text-slate-400 hover:text-primary transition-colors" title="Xem">
                                            <span class="material-symbols-outlined text-[20px]">visibility</span>
                                        </a>
                                        <a href="{{ route('admin.products.comments', $product->id) }}"
                                            class="text-slate-400 hover:text-blue-600 transition-colors" title="Xem bình luận">
                                            <span class="material-symbols-outlined text-[20px]">comment</span>
                                        </a>
                                        <a href="{{ route('admin.products.edit', $product->id) }}"
                                            class="text-slate-400 hover:text-blue-600 transition-colors" title="Sửa">
                            ...
                            <tr>
                                <td colspan="6" class="h-64 text-center text-slate-400 italic bg-white">
                                    <div class="flex flex-col items-center justify-center h-full">
                                        ...
                                        <a href="{{ route('admin.products.create') }}"
                                            class="mt-3 text-primary text-sm font-bold hover:underline">Thêm sản phẩm đầu
                                            tiên</a>
                                    </div>
                                </td>
                            </tr>
